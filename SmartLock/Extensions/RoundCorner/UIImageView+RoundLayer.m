...
#import "UIImageView+RoundLayer.h"
#import "NSObject+MethodSwizzle.h"
#import <objc/runtime.h>

static char NMUIImageViewCornerRadiusKey;
static char WBNSOperationQueueKey;
static CGSize viewSize;

@implementation UIImageView (RoundLayer)

+ (void)load
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        [self swizzleSelector:@selector(setImage:) withSelector:@selector(nm_setImage:)];
    });
}

- (void)setCornerRadius:(CGFloat)radius
{
    objc_setAssociatedObject(self, &NMUIImageViewCornerRadiusKey, [NSNumber numberWithFloat:radius],OBJC_ASSOCIATION_COPY_NONATOMIC);
    viewSize = self.bounds.size;
}

- (CGFloat)radius
{
    NSNumber *value = objc_getAssociatedObject(self, &NMUIImageViewCornerRadiusKey);
...
- (void)nm_setImage:(UIImage *)image
{
    if(self.radius >= 1 && image){
        ...
        [self.cornerOperationQueue addOperationWithBlock:^{
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [self nm_setImage:cornerImage];
//                [self setNeedsDisplay];
            });
        }];
    }
    else{
        [self nm_setImage:image];
    }
}
@end
